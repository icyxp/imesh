...
func main() {
	...
	a := kingpin.New(filepath.Base(os.Args[0]), "The imesh config reloader for k8s")
	a.Version(Version)
	a.HelpFlag.Short('h')
...
func (r *reloader) Run() error {
	...
	err = watcher.Add(r.cfg.configDir)
	if err != nil {
...
func (r *reloader) reload() error {
	req, err := http.NewRequest("POST", r.cfg.reloadURL, nil)
	if err != nil {
		return err
	}
