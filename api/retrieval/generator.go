...
func (g *generator) generateNodeConnectionSet(ctx context.Context, cfgConns []*config.Connection, cfgNotices []*config.NodeNotice, ts time.Time, nodeFactory func(string) *model.Node) (*model.NodeConnectionSet, error) {
	group, groupCtx := errgroup.WithContext(ctx)
	groupConns := make([]([]*model.Connection), len(cfgConns))
	...
	groupNotices := make([](map[string][]*model.Notice), len(cfgNotices))
	...
	for i, cfgConn := range cfgConns {
		for _, conn := range groupConns[i] {
			...
			for _, n := range ns {
				...
				nodeMap[n.Name].NameSpace = "default"
				arr := strings.Split(n.Name, ".")
				if len(arr) > 1 && strings.Contains(arr[1], "-") {
					nodeMap[n.Name].NameSpace = arr[1]
				}
			}
		}
	}
...
func (g *generator) generateConnections(vector prommodel.Vector, conn *config.Connection) []*model.Connection {
	...
	for _, m := range metricMap {
		...
		for _, notice := range conn.Notices {
			...
			t2, err2 := template.New("pql").Parse(notice.PromQL)
			if err != nil || err2 != nil {
				continue
			}
			...
			if pql != "" {
				...
				if err = t2.Execute(&buf2, templateMap); err != nil {
					g.logger.Error("Failed to execute rendering notice template",
						zap.Error(err),
						zap.String("psql", pql),
						zap.Any("templateMap", templateMap))
				}
				link = conn.FormatPromQL(buf2.String())
			} else if link == "" {
				link = conn.QueryLink()
			}
...
func extractNodeName(sample *prommodel.Sample, mapping *config.NodeMapping) (string, error) {
	...
	var pv prommodel.LabelValue
	var ok bool
	var value string

	arr := strings.Split(mapping.Label, ",")
	if len(arr) > 1 {
		for _, v := range arr {
			pv = sample.Metric[prommodel.LabelName(v)]
			if string(pv) == "unknown" {
				value = string(pv)
				break
			}
			value += string(pv) + "."
